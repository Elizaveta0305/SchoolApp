...
namespace SchoolApplication.Tests
{
    public class GradeVmTests
    {
        ...
        public GradeVmTests()
        {
            ...
            _dbContextFactory = new TestDbContextFactory(_testDbName);
        ...
        private async Task<GradeVm> CreateViewModel(User? currentUser = null)
        {
            ...
            if (currentUser != null)
            {
                ...
                vm.Receive(new UserAuthenticatedMessage(userFromDb));
        ...
        public async Task Receive_WithAuthenticatedStudentUser_LoadsStudentDataAndGrades()
        {
            ...
            var vmWithStudent = await CreateViewModel(_studentUser);

            Assert.Equal($"{_studentUser.LastName} {_studentUser.FirstName}", vmWithStudent.StudentFullName);
            Assert.Equal(_group9B.GroupName, vmWithStudent.StudentGroupName);
            Assert.Contains("STM32 в среде STM32CubeIDE", vmWithStudent.StudentSubjects);
            Assert.Contains("Scratch", vmWithStudent.StudentSubjects);
            Assert.NotEmpty(vmWithStudent.StudentGrades);
            Assert.Contains(vmWithStudent.StudentGrades, g => g.PerformanceID == _studentStm32Grade.PerformanceID);
            Assert.Contains(vmWithStudent.StudentGrades, g => g.PerformanceID == _studentScratchGrade.PerformanceID);
            Assert.Equal(2, vmWithStudent.StudentGrades.Count);
        }
        ...
        public async Task LoadStudentDataAndGrades_HandlesNoGrades()
        {
            ...
            var tempTeacherUser = new User { UserID = 112, Username = "tempTeacher", FirstName = "Тест", LastName = "Учитель", MiddleName = "Темп", RoleID = tempTeacherRole.RoleID };
            ...
            var studentWithoutGrades_Test = new User { UserID = 113, Username = "nogrades", FirstName = "Тест", LastName = "БезОценок", RoleID = tempStudentRole.RoleID, GroupID = tempGroup.GroupID };
            ...
            User userWithoutGradesFromDb;
            using (var context = _dbContextFactory.CreateDbContext())
            {
                userWithoutGradesFromDb = await context.Users
                    .Include(u => u.Role)
                    .Include(u => u.Group)
                        .ThenInclude(g => g.StudyGroups!)
                            .ThenInclude(sg => sg.Subject)
                    .FirstOrDefaultAsync(u => u.UserID == studentWithoutGrades_Test.UserID);
            }

            Assert.NotNull(userWithoutGradesFromDb);

            vm.Receive(new UserAuthenticatedMessage(userWithoutGradesFromDb));
            await Task.Delay(500);
        ...
        {
            var studentNoGroup = new User { UserID = 104, Username = "nogroup", FirstName = "Тест", LastName = "БезГруппы", RoleID = _studentRole.RoleID };
            ...
            User userNoGroupFromDb;
            using (var context = _dbContextFactory.CreateDbContext())
            {
                userNoGroupFromDb = await context.Users
                    .Include(u => u.Role)
                    .Include(u => u.Group)
                        .ThenInclude(g => g.StudyGroups!)
                            .ThenInclude(sg => sg.Subject)
                    .FirstOrDefaultAsync(u => u.UserID == studentNoGroup.UserID);
            }
            Assert.NotNull(userNoGroupFromDb);

            vm.Receive(new UserAuthenticatedMessage(userNoGroupFromDb));
            await Task.Delay(500);
